  ...
  return (
    <section className="py-24 bg-brand-cream px-6">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[300px]">
        ...
        {/* Main Origin Card */}
        <motion.div 
          className="md:col-span-8 md:row-span-1 bg-brand-sand/30 rounded-[2.5rem] p-12 flex flex-col justify-end border border-brand-sand/50"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
        >
          <span className="text-brand-leaf font-bold tracking-widest uppercase text-xs mb-4">The Balehonnur Legacy</span>
        ...
        {/* The Chicory Truth Card */}
<motion.div 
  className="md:col-span-4 md:row-span-2 bg-brand-mocha rounded-[2.5rem] p-10 flex flex-col justify-between text-brand-cream"
>
  <div>
    ...
    <p className="text-sm opacity-80 leading-relaxed mb-8">
      ...
      Companies still use it to cut costs, masking the real flavor.
    </p>
    
    {/* ADD THIS BUTTON */}
    <Link href="/heritage" className="inline-flex items-center gap-2 text-brand-sand font-bold text-xs uppercase tracking-widest hover:text-brand-cream transition-colors group">
      Read the full story 
  ...
  <div className="text-5xl opacity-20 font-serif self-end">0%</div>
        ...
        {/* The Tresoro Promise Card */}
        <motion.div 
          className="md:col-span-4 md:row-span-1 bg-brand-forest rounded-[2.5rem] p-10 flex flex-col justify-center text-brand-cream text-center"
        ...
        {/* The Robusta Pride Card */}
        <motion.div 
          className="md:col-span-4 md:row-span-1 bg-brand-sand/10 rounded-[2.5rem] p-10 flex flex-col justify-center border border-brand-sand/30"
          initial={{ opacity: 0, x: 20 }}
          whileInView={{ opacity: 1, x: 0 }}
        >
          <span className="text-brand-leaf font-bold tracking-widest uppercase text-[10px] mb-2">Heritage Grade</span>
